...
def run_scenario(name, duration_sec, delay_range, data_func):
    logger.info(f"---- Starting scenario: {name} for {duration_sec} seconds ----")
    ...
    while time.time() < end_time:
        try:
            ...
            if response.status_code == 200:
                ...
                print(f" {name} | Pred: {pred} | Input: {payload['features']}", end="\r")
            else:
...
if __name__ == "__main__":
    ...
    print("Cycle:Normal->HighLoad->DataDrift->Errors->Normal...")
